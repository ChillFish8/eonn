...
pub unsafe fn f32_xconst_avx512_nofma_euclidean<const DIMS: usize>(
    x: &[f32],
    y: &[f32],
) -> f32 {
...
/// Values within the vector should also be finite, although it is not
/// going to crash the program, it is going to produce insane numbers.
pub unsafe fn f32_xany_avx512_nofma_euclidean(x: &[f32], y: &[f32]) -> f32 {
    debug_assert_eq!(x.len(), y.len());
    ...
    if offset_from != 0 {
        let mut i = offset_from;
        while i > 0 {
            let offset = offset_from - i;
            let (x, y) = load_two_variable_size_avx512(x.add(offset), y.add(offset), i);

            let diff = _mm512_sub_ps(x, y);
            let r = _mm512_mul_ps(diff, diff);
...
mod tests {
    ...
    fn test_xconst_nofma_euclidean() {
        let (x, y) = get_sample_vectors(1024);
        let dist = unsafe { f32_xconst_avx512_nofma_euclidean::<1024>(&x, &y) };
        assert!(is_close(dist, simple_euclidean(&x, &y)));
    }
    ...
    fn test_xany_nofma_euclidean() {
        let (x, y) = get_sample_vectors(563);
        let dist = unsafe { f32_xany_avx512_nofma_euclidean(&x, &y) };
        assert!(is_close(dist, simple_euclidean(&x, &y)));
    }
}
