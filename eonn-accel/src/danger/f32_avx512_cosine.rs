...
pub unsafe fn f32_xconst_avx512_nofma_cosine<const DIMS: usize>(
    x: &[f32],
    y: &[f32],
) -> f32 {
    ...
    let norm_x = f32_xconst_avx512_nofma_norm::<DIMS>(x);
    let norm_y = f32_xconst_avx512_nofma_norm::<DIMS>(y);
    let dot_product = f32_xconst_avx512_nofma_dot::<DIMS>(x, y);
...
/// Values within the vector should also be finite, although it is not
/// going to crash the program, it is going to produce insane numbers.
pub unsafe fn f32_xany_avx512_nofma_cosine(x: &[f32], y: &[f32]) -> f32 {
    debug_assert_eq!(x.len(), y.len());
...
mod tests {
    ...
    fn test_xconst_nofma_cosine() {
        let (x, y) = get_sample_vectors(1024);
        let dist = unsafe { f32_xconst_avx512_nofma_cosine::<1024>(&x, &y) };
        assert_is_close(dist, simple_cosine(&x, &y));
    }
    ...
    fn test_xany_nofma_cosine() {
        let (x, y) = get_sample_vectors(547);
        let dist = unsafe { f32_xany_avx512_nofma_cosine(&x, &y) };
        assert_is_close(dist, simple_cosine(&x, &y));
    }
}
